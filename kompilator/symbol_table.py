...
class Array:
    ...
    def get_at(self, index):
        ...
        if 0 <= index <= self.last_index - self.first_index + 1:
...
class SymbolTable(dict):
        ...
        if type(target) == str:
            ...
            if(target[0] == 'array'):
                if(type(target[2]) == tuple):
                    trg = target[2][1][1]
                    return self.get_array_at(target[1], trg)
                else:
                    return self.get_array_at(target[1], target[2])
            else:
    ...
    def change_iterator(self, name, newofset, newlimit):
        if name in self.iterators:
            iterator = self.iterators[name]
            iterator.memory_offset = newofset
            iterator.limit_address = newlimit
